...
class NotificationController extends GetxController {
  RxList<Map<String, dynamic>> allNotifications = <Map<String, dynamic>>[].obs; // 전체 알림 데이터
  RxList<Map<String, dynamic>> unreadNotifications = <Map<String, dynamic>>[].obs; // 읽지 않은 알림 데이터
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (responseBody != null && responseBody.isNotEmpty) {
          unreadNotificationCount.value = int.tryParse(responseBody) ?? 0; // Null-safe 파싱
        } else {
          unreadNotificationCount.value = 0; // 응답이 비어 있을 경우 0으로 설정
        }
      } else {
        print('Failed to fetch unread notification count: ${response.statusCode}');
      }
    } catch (e) {
  ...
  Future<void> fetchNotifications() async {
    try {
      final response = await http.get(Uri.parse('http://3.34.102.55:8080/member/1/notification'));
      if (response.statusCode == 200) {
        ...
        allNotifications.value = data.map((item) {
          ...
          return {
            ...
            'date': item['createdAt'] != null ? DateTime.parse(item['createdAt']).toString() : '날짜 없음',
            'category': category,
  ...
  Future<void> markAsRead(int notificationId) async {
    try {
      final response = await http.put(
        Uri.parse('http://3.34.102.55:8080/member/1/notification/$notificationId/read'),
      );
      if (response.statusCode == 200) {
        // 읽음 상태를 업데이트
        fetchNotifications();
